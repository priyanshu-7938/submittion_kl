...
class DataLayer {
    ...
    private constructor() {
        if(!process.env.REDIS_URL)throw Error("Cant locate Redis-url in env");
        this.config = {
    ...
    async getKnowledgeContext( // done!
        query: string,
    ): Promise<{ context: string; source: "cache" | "database" }> {
 ...
 */
    async getChatHistory(sessionId: string): Promise<Message[]> {
        try {
            ...
            const cachedMessages: Message[] = messages.map((msg) => ({
                id: msg.id,
                role: msg.role,
                content: msg.content,
 ...
 */
    async addChatMessages(
        ...
        try {
            ...
            const cachedMessages: Message[] = dbMessages
                .reverse()
                .map((msg) => ({
                id: msg.id,
                role: msg.role,
            ...
            if (this.isConnected) {
                ...
                cacheKey,
                this.config.chatHistoryTTL,
                JSON.stringify([...existingHistory, ...cachedMessages])
                );
            }
...
export type { Message, CachedKnowledge };