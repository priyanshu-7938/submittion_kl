...
class KnowledgeService {
  ...
  async ingestDocument(fullText: string): Promise<void> {
  ...
  async searchSimilar(query: string): Promise<string> {
    try {
      const result = await this.model.embedContent(query);
      console.log(this.model);
      const queryVector = result.embedding.values;
      
      const results = await this.prisma.$queryRaw<Array<{ content: string }>>`
        SELECT content 
        FROM knowledge_base 
      ...
      return results.map((row: any) => row.content).join("\n---\n");
    } catch (error) {
      console.error(error);
