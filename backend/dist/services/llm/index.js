...
class GeminiClient {
    ...
    buildSystemPrompt(companyName, customInstructions) {
...
Remember: Always prioritize user satisfaction and accurate information delivery.`;
    ...
    */
    async generateReply(history, context, query) {
        try {
     ...
     */
    buildEnhancedPrompt(context, query) {
     ...
     */
    async generateSimpleReply(query, context) {
        try {
     ...
     */
    async *generateStreamReply(history, context, query) {
        try {
            const enhancedPrompt = this.buildEnhancedPrompt(context, query);
            const formattedHistory = this.formatHistory(history);
            const chat = this.model.startChat({
